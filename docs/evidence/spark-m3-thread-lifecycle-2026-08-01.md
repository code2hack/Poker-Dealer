...
was exhaustive authoritative list/read state. After an accepted Archive or Delete, Dealer now uses
an exhaustive all-source readback to identify exactly which reviewed host-qualified locators
actually changed, then persists the corresponding detach or purge. Server lifecycle notifications
remain an authoritative, idempotent path. Compatibility fixtures and state tests cover both
notification and readback reconciliation, including non-user descendants and partial cascades.
