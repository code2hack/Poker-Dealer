...
class PokerInputAdapterTest {
    ...
    fun `touch swipes map to canonical operations with monotonic duration`() {
        ...
        assertTrue(adapter.onTouchEvent(touch(PokerTouchAction.DOWN, y = 0f, time = 10)).isEmpty())
        val begin = adapter.onTouchEvent(touch(PokerTouchAction.MOVE, y = 30f, time = 20)).single()
        ...
        assertEquals(PokerOperation.DOWN, begin.interaction.operation)
    ...
    fun `two-finger tap is emitted once and repeat function downs are ignored`() {
        ...
        assertEquals(listOf(PokerInteractionPhase.BEGIN, PokerInteractionPhase.RELEASE), tap.map { it.interaction.phase })
        assertEquals(PokerOperation.TAPTAP, tap.first().interaction.operation)
    ...
    private fun adapter(nowMs: Long = 0) = PokerBuiltInInputAdapter(
