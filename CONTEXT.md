...
M3 is complete. The active slice is the expanded M4: code-only Dealer↔Poker PAKE pairing with temporary Android NSD/mDNS endpoint discovery, secure Wi-Fi synchronization, horizontal card-pile navigation, canonical controls and bindings, reviewed composer and request input, Photo, Morse, and Dealer-local ASR. The former M5 milestone has been merged into M4 and retired.
...
- Dealer owns configured hosts, route priority, embedded-tailnet lifecycle, distribution-specific daemon behavior, mobile presentation, recent projection/cache, unread state, control-surface intent, durable composer drafts and photo assets, Dealer-local ASR inference and provisional transcripts, the pinned Poker pairing record/current authenticated Wi-Fi endpoint, and Poker synchronization.
...
Several clients may observe one thread. Only one human-control surface should actively submit turns, steer, interrupt, or resolve a server request at a time.
...
- **Poker pairing:** The one-to-one trust relationship between one Dealer installation and one Poker installation. Poker opens a five-minute enrollment window only after the user physically chooses Pair Dealer or Replace Dealer; Poker shows only a six-digit one-time code, Dealer accepts only that code, and temporary Android NSD/mDNS discovery supplies the current Wi-Fi endpoint without making discovery itself trusted. The PAKE exchange pins both Android-Keystore-backed app identities; authenticated reconnection reuses that relationship, and trusting a different Dealer requires explicit physical replacement.
...
- **Poker Primary action:** The action wheel's context-sensitive action: Send for a nonempty Ready composer, Steer for a nonempty Busy composer, semantic Interrupt when the Busy composer is empty, or submit the current valid response from a request panel Poker may safely resolve.
